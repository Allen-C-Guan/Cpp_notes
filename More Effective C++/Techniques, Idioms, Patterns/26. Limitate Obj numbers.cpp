 ...
 *
 * 这里还有一个牛逼的地方，就是这里采用template作为counted的接口，这会让各个类之间不共享maxCounted的个数！！，但同类型之间是共享次数的！！
 * 比如：class Printer : private Counted<Printer> {...} 和
 * 和 class Computer : private Counted<Computer> {...} 之间就是不共享counted个数的
 *
