 ...
 * 2. 非模板函数：指的是如果匹配质量相同，则模板函数优先非模板函数（非模板函数更特殊一点，否则非模板函数用于不会被调用到！）
 ...
 * }
 *
 * ********************* 为何C++不允许函数的部分特例化，却允许类的部分特例化？***************************
 * 简言之：函数的部分特例化和函数的重载之间在函数匹配的过程中会有冲突，往往得不到预期结果，但是类模板就没有这个问题。
 * 因为类不可以重载！！！但函数可以重载！！
 *
 * 一方面，你不能偏特化基模板 -- 仅仅是因为标准委员会不允许；另一方面，函数模板的特化函数不参与重载，
 * 这意味着你写的特化函数并不影响编译器选择哪个基模板（这也正是最不直观的地方）。如果你写了个非模板函数，
 * 在参数合适的情况下编译器会优先选择这个它。
 *
 *
 */
